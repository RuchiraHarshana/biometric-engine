...
export function setAuthTokens(data: { access_token: string; role: string; email: string }) {
  if (typeof window === "undefined") return
...
export async function verify2faApi(code: string) {
  ...
  return res.json() as Promise<{ access_token: string; token_type: string; role: string; email: string }>
}
